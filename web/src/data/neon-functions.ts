...
export const neonFunctions: NeonFunction[] = [
  ...
  },
  {
    ...
    notes: ["true does not mean the destination has been reached; the API has no completion event or durable task handle.", "The focused route harness validates the drive_to sequence form. The standalone call compiled successfully but was not exercised separately in game."],
    nativeTask: {
      tasks: ["CTaskComplexCarDriveToPoint"], opcode: "05D1", command: "TASK_CAR_DRIVE_TO_COORD",
      note: "Neon calls GTA's original constructor at 0x63CE00. The direct form supplies the current vehicle, desired model -1, and native radius -1.0 before queuing the task through GTA's script-command path.",
    },
