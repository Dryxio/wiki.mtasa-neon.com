...
export const neonCategories = {
  ...
  effects: {
    ...
    test: "test-resources/2dfx-test",
    lifecycle: "Custom effects belong to the resource that added them. Overrides and removals of original GTA effects are stacked per resource, so two resources editing the same model do not clobber each other, and stopping one rolls back only its own layer.",
  },
  birds: {
  ...
  fire: {
    title: "Managed fire",
    guide: "/neon/fire",
    test: "test-resources/fire-test",
    lifecycle: "A managed fire is an element owned by the resource that created it, so stopping that resource extinguishes its fires. Server-created fires are synchronized to every client, and lifetime is tracked as relative remaining time so a client joining mid-burn receives what is left.",
  },
  foliage: {
    title: "Custom foliage",
    guide: "/neon/foliage",
    test: "test-resources/foliage-test",
    lifecycle: "A foliage element belongs to the resource that created it and joins its element group, so stopping the resource removes its patches without explicit cleanup. Every setter rebuilds the native triangle atomically and rolls back to the previous state when the new combination is rejected.",
  },
  collision: {
...
export const neonFunctions: NeonFunction[] = [
...

  {
    ...
    summary: "Send an NPC straight toward a nearby coordinate. GTA handles the walk or run, slows the NPC down, and stops it inside the arrival radius.",
    ...
    notes: ["This is GTA's straight-to-coordinate task. For a route through GTA's pedestrian path network and around buildings, use setPedNavigateTo."],
    example: "-- client.lua\n-- Sweet runs directly to a nearby point and stops there.\nlocal sweet = createPed(270, 2495.2, -1687.4, 13.5)\nlocal destination = Vector3(2520.0, -1672.0, 13.8)\n\nif sweet then\n    assert(setPedGoTo(sweet, destination, \"run\", 0.75, 3.0))\nend",
    featuredExample: true,
  },
  {
    name: "setPedNavigateTo", category: "tasks", side: "client", group: "movement",
    signature: "bool setPedNavigateTo(ped thePed, Vector3 target [, string movement = \"walk\", float radius = 0.5, float slowdownRadius = 3.0, float heightChangeThreshold = 2.0, bool keepNodesHeadingAwayFromTarget = false, int timeout = -1, bool useBlending = true])",
    summary: "Give an NPC a destination and GTA's pedestrian pathfinder takes over: it chooses a route through the native path network, goes around buildings, walks or runs naturally, slows down, and stops near the target.",
    arguments: [arg("thePed", "ped", "Living streamed ped simulated by this client."), arg("target", "Vector3", "Finite world destination."), arg("movement", "string", "walk, run, or sprint.", true, "walk"), arg("radius", "float", "Positive arrival radius.", true, "0.5"), arg("slowdownRadius", "float", "Slowdown radius not smaller than radius.", true, "3.0"), arg("heightChangeThreshold", "float", "Positive vertical-change threshold used while GTA builds the route.", true, "2.0"), arg("keepNodesHeadingAwayFromTarget", "bool", "Keep route nodes that initially head away from the target when GTA needs them.", true, "false"), arg("timeout", "int", "-1 for no timeout, or a non-negative timeout in milliseconds.", true, "-1"), arg("useBlending", "bool", "Blend the native movement transitions.", true, "true")],
    returns: "true when GTA accepted the route task; false when the ped, ownership, streaming, liveness, destination, movement, radii, height threshold, or timeout is invalid.",
    notes: ["true means the task was accepted, not that the NPC has arrived. There is no completion event or durable route handle."],
    oop: ["ped:setNavigateTo(target, movement, radius, slowdownRadius, heightChangeThreshold, keepNodesHeadingAwayFromTarget, timeout, useBlending)"],
    nativeTask: {
      tasks: ["CTaskComplexFollowNodeRoute"],
      note: "GTA builds and follows its pedestrian node route. Neon also preserves the task's native recovery when the ped meets a building collision. The audited source identifies task type 906; no numeric SCM opcode is claimed.",
    },
    source: "Client/mods/deathmatch/logic/luadefs/CLuaPedDefs.cpp", commit: "7b4795ee3", test: "test-resources/native-ped-navigate-harness",
    example: "-- client.lua\n-- GTA chooses the pedestrian route; Lua only gives the destination.\nlocal sweet = createPed(270, 2495.2, -1687.4, 13.5)\nlocal destination = Vector3(2520.0, -1672.0, 13.8)\n\nif sweet then\n    assert(setPedNavigateTo(sweet, destination, \"run\"))\nend",
    featuredExample: true,
  },
  {
  ...
  {
    name: "setPedDriveMission", category: "tasks", side: "client", group: "driving",
    signature: "bool setPedDriveMission(ped thePed, vehicle theVehicle, vehicle targetVehicle, string|int mission, float speed [, string|int drivingStyle = \"stop_for_cars\"])",
    summary: "Tell an NPC driver to escort another vehicle from its left side using GTA's original mission-driving AI.",
    arguments: [arg("thePed", "ped", "Living streamed ped simulated by this client."), arg("theVehicle", "vehicle", "Streamed, non-blown vehicle occupied and controlled by the ped."), arg("targetVehicle", "vehicle", "Different streamed, non-blown vehicle to escort."), arg("mission", "string|int", "escort_left or its native ID 29."), arg("speed", "float", "Finite cruise speed from 0 up to, but not including, 255."), arg("drivingStyle", "string|int", "Integer 0..6 or a documented driving-style name.", true, "stop_for_cars")],
    returns: "true when GTA accepted the escort task; false when ownership, vehicles, seat, mission, speed, style, or native task construction is invalid.",
    notes: ["escort_left is the only mission mode currently admitted. true means the task was accepted, not that the escort is complete."],
    nativeTask: { tasks: ["CTaskComplexCarDriveMission"], command: "MISSION_ESCORT_LEFT", note: "Neon currently admits only GTA mission ID 29, the escort-left behavior verified by the Sweet & Kendl checkpoint." },
    source: "Client/mods/deathmatch/logic/luadefs/CLuaPedDefs.cpp", commit: "cf508dcec", test: "test-resources/native-task-cohort-test",
    example: "-- client.lua\nlocal escortCar = createVehicle(412, 2490, -1685, 13.4)\nlocal targetCar = createVehicle(492, 2500, -1685, 13.4)\nlocal driver = createPed(105, 2490, -1685, 13.4)\n\nwarpPedIntoVehicle(driver, escortCar, 0)\nassert(setPedDriveMission(driver, escortCar, targetCar, \"escort_left\", 18, \"avoid_cars\"))",
  },
  {
  ...
  {
    name: "setPedPhysicalProofs", category: "tasks", side: "client", group: "policies",
    signature: "bool setPedPhysicalProofs(ped thePed, bool bullet, bool fire, bool explosion, bool collision, bool melee)",
    summary: "Choose exactly which kinds of physical damage an NPC ignores: bullets, fire, explosions, collisions, or melee hits.",
    arguments: [arg("thePed", "ped", "Script ped controlled by this client; players are rejected."), arg("bullet", "bool", "Bullet-proof flag."), arg("fire", "bool", "Fire-proof flag."), arg("explosion", "bool", "Explosion-proof flag."), arg("collision", "bool", "Collision-proof flag."), arg("melee", "bool", "Melee-weapon-proof flag.")],
    returns: "true when the current ped syncer stored the complete proof tuple; false for a player, invalid ped, or observer client.",
    notes: ["The policy survives stream-out and native ped recreation on this client.", "It has no automatic resource-stop restoration. Save the previous values with getPedPhysicalProofs and restore them during cleanup."],
    nativeTask: { command: "SET_CHAR_PROOFS", note: "This does not create a CTask. It writes GTA's five independent ped-proof flags and keeps them on the MTA ped for native recreation." },
    source: "Client/mods/deathmatch/logic/luadefs/CLuaPedDefs.cpp", commit: "cf508dcec", test: "test-resources/native-task-cohort-test",
    example: "-- Temporarily make a mission actor fireproof.\nlocal bullet, fire, explosion, collision, melee = getPedPhysicalProofs(actor)\nassert(setPedPhysicalProofs(actor, bullet, true, explosion, collision, melee))\n\n-- Restore this tuple when the scene ends.\nsetPedPhysicalProofs(actor, bullet, fire, explosion, collision, melee)",
  },
  {
    name: "getPedPhysicalProofs", category: "tasks", side: "client", group: "policies",
    signature: "bool, bool, bool, bool, bool getPedPhysicalProofs(ped thePed)",
    summary: "Read the NPC's five stored physical-damage protections so a resource can change them temporarily and restore them later.",
    arguments: [arg("thePed", "ped", "Ped whose local proof tuple should be read.")],
    returns: "Five booleans in bullet, fire, explosion, collision, and melee order. An invalid ped returns five false values.",
    nativeTask: { note: "This is a policy query, not a CTask or SCM command. It reads the tuple persisted on the MTA ped." },
    source: "Client/mods/deathmatch/logic/luadefs/CLuaPedDefs.cpp", commit: "cf508dcec", test: "test-resources/native-task-cohort-test",
  },
  {
  ...
  {
    name: "setVehicleStraightLineDistance", category: "vehicle", side: "client",
    signature: "bool setVehicleStraightLineDistance(vehicle theVehicle, int distance)",
    summary: "Tune GTA's built-in straight-line distance value for a vehicle's native autopilot, as used by original mission driving.",
    arguments: [arg("theVehicle", "vehicle", "Vehicle controlled by this client."), arg("distance", "int", "Native autopilot value from 0 through 255.")],
    returns: "true when the locally owned vehicle stored the value; false for an observer, invalid vehicle, or value outside 0..255.",
    notes: ["The value survives stream-out and native vehicle recreation on this client.", "This is a low-level GTA autopilot setting, not a world distance in metres. Save and restore the previous value when a task ends."],
    nativeTask: { opcode: "04E0", note: "This does not create a CTask. It writes CAutoPilot::m_nStraightLineDistance, the byte used by GTA's vehicle AI." },
    source: "Client/mods/deathmatch/logic/luadefs/CLuaVehicleDefs.cpp", commit: "cf508dcec", test: "test-resources/native-task-cohort-test",
    example: "local previous = getVehicleStraightLineDistance(missionVehicle)\nassert(setVehicleStraightLineDistance(missionVehicle, 80))\n\n-- Restore it after the mission task.\nif previous ~= false then\n    setVehicleStraightLineDistance(missionVehicle, previous)\nend",
  },
  {
    name: "getVehicleStraightLineDistance", category: "vehicle", side: "client",
    signature: "int|false getVehicleStraightLineDistance(vehicle theVehicle)",
    summary: "Read the vehicle's current GTA autopilot straight-line distance value so it can be restored after a mission task.",
    arguments: [arg("theVehicle", "vehicle", "Vehicle whose native or stored value should be read.")],
    returns: "The current value from 0 through 255, or false when no value is available.",
    notes: ["The function reads the live GTA vehicle when streamed in and the locally stored override otherwise."],
    nativeTask: { note: "This is a CAutoPilot state query, not a CTask or SCM command." },
    source: "Client/mods/deathmatch/logic/luadefs/CLuaVehicleDefs.cpp", commit: "cf508dcec", test: "test-resources/native-task-cohort-test",
  },
  ...
  {
    name: "setVehiclePlaybackSpeed", category: "recording", side: "client",
    signature: "bool setVehiclePlaybackSpeed(vehicle theVehicle, float speed)",
    summary: "Speed up, slow down, or pause a vehicle while it follows its active GTA recorded route.",
    arguments: [arg("theVehicle", "vehicle", "Streamed vehicle whose active playback belongs to the calling resource."), arg("speed", "float", "Playback multiplier from 0 through 16; 1 is normal speed and 0 pauses movement.")],
    returns: "true when the active native playback accepted the speed; false for invalid input, inactive or foreign playback, or a vehicle not controlled by this client.",
    notes: ["The calling resource must own the active playback slot. Stream-out, ownership loss, vehicle destruction, or resource shutdown still stops playback."],
    nativeTask: { opcode: "06FD", command: "SET_PLAYBACK_SPEED", note: "GTA changes the speed of the active recorded-car slot; Neon requires both the playback lease and current vehicle authority." },
    source: "Client/mods/deathmatch/logic/luadefs/CLuaVehicleDefs.cpp", commit: "c21792236", test: "test-resources/native-vehicle-recording-test",
    example: "assert(startVehiclePlayback(vehicle, recordingId))\nassert(setVehiclePlaybackSpeed(vehicle, 2.0)) -- follow the same route at 2x speed",
  },
  {
