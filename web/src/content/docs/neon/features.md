...
| Give custom vehicles their own sound | [Custom vehicle audio](/neon/vehicle-audio) | Convert a vehicle sound set—an Assetto Corsa engine pack, for example—into the expected banks and assign it to a model. Neon adapts it to RPM, throttle and gears, and can add automatic or Lua-triggered backfire sounds and exhaust flames. |
