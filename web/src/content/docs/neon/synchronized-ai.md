...
When a new traffic car starts driving, Neon compares nearby roads with the car's actual position, forward direction, and height. It chooses the road that best matches the vehicle, keeps one-way lane directions legal, and avoids attaching a correctly aligned car to a nearby crossing or road on another level. This reduces immediate U-turns, wrong-way starts, and bad junction choices.

The complete car and its occupants remain invisible during their short setup. GTA first settles the suspension on the road surface, confirms ground contact, installs `DriveWander`, and applies the road-aligned starting speed. Only then does the server reveal the unit. A candidate that cannot settle safely is recycled instead of appearing frozen or embedded in a slope.
...
The reference resource creates traffic around players in shared 180-unit areas. Nearby players share the same cars instead of receiving duplicate local traffic. Its production profile targets 16 vehicles per active area and allows no more than 160 across the server. Walking population keeps GTA's civilian, gang, cop, and dealer proportions, doubles the native zone target, then clamps populated areas to 12–20 walkers with a global cap of 240. A zero-population zone remains empty.

For a player moving quickly, both population resources look a short, bounded distance ahead—at most 45 world units—when asking for the next candidates. Road traffic can also begin replacing cars that are already falling behind. This helps the next area fill before the player arrives without permanently raising the configured density.

The pedestrian profile also reads GTA's percentage value with its real integer return convention. The previous calling mismatch could eventually turn valid population targets into stale or invalid values and stop new walkers from being requested; the corrected read keeps the refill calculation usable.
...
The later population work is tracked by [`1004257b9`](https://github.com/Dryxio/mtasa-neon/commit/1004257b9), [`39f782fd9`](https://github.com/Dryxio/mtasa-neon/commit/39f782fd9), [`75dd85f77`](https://github.com/Dryxio/mtasa-neon/commit/75dd85f77), [`9fa50ed6a`](https://github.com/Dryxio/mtasa-neon/commit/9fa50ed6a), [`53478fdb3`](https://github.com/Dryxio/mtasa-neon/commit/53478fdb3), [`083927c23`](https://github.com/Dryxio/mtasa-neon/commit/083927c23), [`3accf283f`](https://github.com/Dryxio/mtasa-neon/commit/3accf283f), [`5318f4360`](https://github.com/Dryxio/mtasa-neon/commit/5318f4360), [`500d3cf7e`](https://github.com/Dryxio/mtasa-neon/commit/500d3cf7e), and [`d709fa696`](https://github.com/Dryxio/mtasa-neon/commit/d709fa696). Synchronized road traffic was added in [`12e5799`](https://github.com/Dryxio/mtasa-neon/commit/12e5799) and completed in [`b23f57d`](https://github.com/Dryxio/mtasa-neon/commit/b23f57d), with production ownership, scaling, and telemetry added in [`9e2613e`](https://github.com/Dryxio/mtasa-neon/commit/9e2613e), [`145d31a`](https://github.com/Dryxio/mtasa-neon/commit/145d31a), [`3222987`](https://github.com/Dryxio/mtasa-neon/commit/3222987), and [`cad43e835`](https://github.com/Dryxio/mtasa-neon/commit/cad43e835). The current road selection, density, hidden setup, slope settling, look-ahead refill, and population-profile fixes are tracked by [`977acff28`](https://github.com/Dryxio/mtasa-neon/commit/977acff28), [`2fa523c3e`](https://github.com/Dryxio/mtasa-neon/commit/2fa523c3e), [`c9e8a0fa8`](https://github.com/Dryxio/mtasa-neon/commit/c9e8a0fa8), [`66b919d48`](https://github.com/Dryxio/mtasa-neon/commit/66b919d48), [`15d5c78b1`](https://github.com/Dryxio/mtasa-neon/commit/15d5c78b1), [`1879a4db6`](https://github.com/Dryxio/mtasa-neon/commit/1879a4db6), [`93912d394`](https://github.com/Dryxio/mtasa-neon/commit/93912d394), and [`b5295d493`](https://github.com/Dryxio/mtasa-neon/commit/b5295d493).

Two-client runs cover pedestrian density, group handoff, combat, motorcycle carjacks, dealers, cops, couples, civilian vehicle creation, native driving, passenger entry, ownership handoff, stuck recovery, destruction, and deterministic cleanup. The first 12–20/160/240 scaling change was Lua-checked without a runtime density pass. A later FreeROAM pass observed 17 newly created traffic actors settling on the ground and the local vehicle area refilling to 16/16. After the native percentage-return fix, Game SA and Client Deathmatch built and a FreeROAM connection observed valid civilian, cop, and dealer targets without the earlier `target-invalid` rejection. These checks do not prove saturation at both global caps on a public server.
